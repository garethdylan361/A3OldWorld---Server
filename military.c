...
void generate_demon_mission(int lvl,struct military_ppd *ppd)
{
	...
	ppd->mis[3].opt2=min(118,lvl);				// at level lvl+1
	...
	ppd->mis[4].opt2=min(118,lvl);				// at level lvl+2
...
void generate_mission_lvl(int cn,struct military_ppd *ppd, int lvl)
{
	generate_demon_mission(lvl,ppd);
	ppd->mission_yday=yday+1;
}
...
void military_master_driver(int cn,int ret,int lastact)
{
	...
	for (msg=ch[cn].msg; msg; msg=next) {
		...
		if (msg->type==NT_CHAR) {
			...
			if (ppd->solved_mission) {
				...
				if (ch[co].prof[P_MERCENARY]) {
					int gold;
					gold = ppd->mis[n].exp/5;
					ch[co].gold+= gold > 500000 ? 500000 : gold;
					ch[co].flags|=CF_ITEMS;
					say(cn,"Well done, %s. You've solved your mission! Your pay is %.2fG.",ch[co].name,gold/100.0);
		...
		if (msg->type==NT_TEXT) {
			...
			if ((ptr=strcasestr((char*)msg->dat2,"level:"))) {
				char temp[3];
				int level;
				ptr += 6;
