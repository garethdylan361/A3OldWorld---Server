...
static int cmd_complain(int cn,char *ptr)
{
	...
	if (!ppd->complaint_date) {
		log_char(cn,LOG_SYSTEM,0,"Complaints are meant as a way to complain about verbal attacks by another player, or to report a scam. If you wish to complain about something else, please email game@astonia.com. No complaint has been sent. Repeat the command if you still want to send your complaint.");
...
static int cmd_punish(int cn,char *ptr)		// 1=OK, 0=repeat
{
	...
	task_punish_player(uID,ch[cn].ID,level,reason);
        ...
        sprintf(buf,"00000000003Punishment: %s (%s) scheduled punishment for %s, level %d, reason: %s.",ch[cn].name,ch[cn].staff_code,name,level,reason);
        server_chat(31,buf);
...
static int cmd_shutup(int cn,char *ptr)		// 1=OK, 0=repeat
{
	...
	server_chat(1034,buf);
        ...
        sprintf(buf,"00000000003Shutup: %s (%s) scheduled shutup for %s, %d minutes.",ch[cn].name,ch[cn].staff_code,name,minutes);
        server_chat(31,buf);
...
static int cmd_exterminate(int cn,char *ptr)
{
	...
	sprintf(buf,"00000000003Exterminate: %s (%s) scheduled exterminate for %s.",ch[cn].name,ch[cn].staff_code,name);
	server_chat(31,buf);
...
static int cmd_tell(int cn,char *ptr)
{
	...
	tell_chat(ch[cn].ID,uID,(ch[cn].flags&(CF_STAFF|CF_GOD)) ? 1 : 0," %s %s%s%s (%d) tells you: \"%s\"",
		sname,
...
static void cmd_help(int cn)
{
	...
	log_char(cn,LOG_SYSTEM,0,"/joinall <nr> - joins all chat channel <nr>");
	...
	if (ch[cn].flags&CF_GOD) {
		...
		log_char(cn,LOG_SYSTEM,0,"/createorb <skill>");
...
int command(int cn,char *ptr)	// 1=ok, 0=repeat
{
	...
	if ((len=cmdcmp(ptr,"playerkiller",12))) {
			...
			else log_char(cn,LOG_SYSTEM,0,"Please take a moment to consider this decision. If another player kills you, he will be able to take all your belongings, or kill you over and over again. Do you really want this? Type: '/iwilldie %d' to confirm.",ch[cn].ID);
		}
	...
	if ((len=cmdcmp(ptr,"createorb",7)) && (ch[cn].flags&CF_GOD)) {
		int in = 0;
		...
		in=create_orb_by_name(ptr);

		if (!in) {
			log_char(cn,LOG_SYSTEM,0,"Nothing happens.");
			return 1;
		} else {
			log_char(cn,LOG_SYSTEM,0,"An %s was created.",it[in].name);
		}

		if (ch[cn].flags&CF_PLAYER) dlog(cn,in,"took from orbspawner");
		ch[cn].citem=in;
		ch[cn].flags|=CF_ITEMS;
		it[in].carried=cn;
		return 1;
	...
	if ((len=cmdcmp(ptr,"noexp",3))) {
		ch[cn].flags^=CF_NOEXP;
                log_char(cn,LOG_SYSTEM,0,"Turned no-exp mode %s.",(ch[cn].flags&CF_NOEXP) ? "on" : "off");
		update_char(cn);
                return 1;
	}

	if ((len=cmdcmp(ptr,"nohints",3))) {
		ch[cn].flags^=CF_NOHINTS;
                log_char(cn,LOG_SYSTEM,0,"Turned no-hint mode %s.",(ch[cn].flags&CF_NOHINTS) ? "on" : "off");
		update_char(cn);
                return 1;
	}
    ...
    if ((len=cmdcmp(ptr,"givemili",8)) && (ch[cn].flags&(CF_GOD))) {
		int exp;

		ptr+=len;
		exp=atoi(ptr);
		give_military_pts_no_npc(cn, exp, 0);
