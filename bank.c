		...
		if (msg->type==NT_TEXT) {
			analyse_text_driver(cn,msg->dat1,(char*)msg->dat2,msg->dat3);
				...
				if (ppd && (ptr=strcasestr((char*)msg->dat2,"deposit"))) {
					...
					val=atoi(ptr);
                                        if (val) {
						if (val>(ch[co].gold/100) || val<1) {
							quiet_say(cn,"Thou dost not have that much gold.");
						} else {
							ch[co].gold-=val*100;
							ppd->imperial_gold+=val*100;
							ch[co].flags|=CF_ITEMS;
							quiet_say(cn,"Thou hast deposited %d gold coins.",val);
							dlog(co,0,"deposited %d, new balance %d.",val,ppd->imperial_gold/100);
						}
